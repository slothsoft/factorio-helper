...
public class Accordion extends JPanel {
	...
	private Image createClosedImage(int w, int h) {
	...
	private Graphics2D createGraphics(BufferedImage result) {
	...
	private Image createOpenImage(int w, int h) {
		final BufferedImage result = new BufferedImage(w, h, BufferedImage.TYPE_INT_RGB);
		final Graphics2D graphics = createGraphics(result);
	...
	private GridBagData createNextGridBagData() {
		return GridBagData.forLabel(0, getComponentCount()).fill(GridBagConstraints.HORIZONTAL);
	}
